 ...
 */
function shouldSendOverdueNotification(service, now) {
    if (!service.lastServiceNotificationSent)
        return true;
    ...
    try {
        ...
        for (const userDoc of usersSnapshot.docs) {
            ...
            for (const carDoc of carsSnapshot.docs) {
                ...
                const serviceSnapshot = await sendNotification_1.db.collection("service-records")
                    .where("carId", "==", car.id)
                    .get();
